...
func GenerateInspirationText() (string, error) {
	...
	reqBody := ChatRequest{
		...
		Messages: []ChatMessage{
			{
				Role:    "system",
				Content: "你是艺术文案创作助手。以月光淬炼的意象为针，缝合梦境与现实的罅隙，编织三行关于存在的温柔悖论。不需要标题或解释，直接输出精炼的艺术灵感短句，必须具有诗意和启发性，适合用作配色创作的灵感来源。",
			},
			{
				Role:    "user",
				Content: "随机种子" + time.Now().Format(time.RFC3339Nano),
			},
		},
		Temperature: 1.1,
		MaxTokens:   80,
	...
	if content == "" {
		return "", fmt.Errorf("empty inspiration content")
	}
